...
const getAllPosts = async (req, res) => {
  ...
  const filteredPosts = allPosts.filter((post) =>
    filteredUsers.includes(post.email)
  );
  const unFilteredPosts = allPosts.filter(
    (post) => !filteredUsers.includes(post.email)
  );

  const unfiltered = await Posts.findOneAndRemove({
    email: unFilteredPosts.map((poster) => poster.email),
  });
...
const updatePost = async (req, res) => {
  ...
  try {
    const foundPost = await Posts.findByIdAndUpdate(req.params.id, editedPost, {
      new: true,
    });
...
const likePost = async (req, res) => {
  ...
  try {
    ...
    const likeCheck = postToLike.likeCount.some(
      (like) => like.username === username
    );

    if (likeCheck) {
      const numberOfLikes = [
        ...new Set(postToLike.likeCount.map((post) => post.username)),
      ];
      return res.json({
...
const userComment = async (req, res) => {
  ...
  try {
    ...
    if (!commentByUser)
      return res.json({ message: "No comments with such id" });
